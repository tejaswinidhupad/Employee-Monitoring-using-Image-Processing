...
/* 	        $(".sideM").removeClass('active');
	    ...
	    function leaveResponse(lid,leaveRes){
	    	...
	    	var type= $(".type-"+lid).text();

	    	//alert(date);
	    	$.ajax({
    		   url: 'LeaveApproveservlet',
    		   data: {lid:lid,leaveRes:leaveRes,date:date,eid:eid,type:type},
    		   type: 'GET',
    		   dataType: 'json',
    		   error: function() {
				...
				<table class="table table-bordered">
					...
					<tbody>
						<%
							...
							ResultSet rs = db
									.selectQuery("select leave_request.*,employee.name,employee.position from leave_request,employee where leave_request.status='"
											+ "pending"
											+ "' and employee.eid=leave_request.eid");
						...
						<tr>
							...
							<td class="type-<%=rs.getInt("id")%>"><%=rs.getString("type")%></td>
							 <td class="status-<%=rs.getInt("id")%>">pending</td>
							
							<td><button type="button"
									onclick='leaveResponse(<%=rs.getInt("id")%>,1)'
									class="btn btn-sm btn-success">Approve</button>
								<button type="button"
									onclick='leaveResponse(<%=rs.getInt("id")%>,0)'
									class="btn btn-sm btn-danger">Disapprove</button></td>
						</tr>
						<%
							}
							}
